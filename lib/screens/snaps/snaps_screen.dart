...
class _SnapsScreenState extends State<SnapsScreen> {
  ...
  void _loadData() {
    ...
    setState(() {
      _snaps = List.of(rawSnaps)..shuffle();
    });
  ...
  void _handleAutoScroll(int feedIndex, VideoPlayerController controller) {
    ...
    _autoAdvancedIndices.add(feedIndex);
    final nextIndex = _focusedIndex + 1;
    final hasNext =
        _isAdIndex(nextIndex) ||
        _videoIndexForFeedIndex(nextIndex) < _snaps.length;
    if (!hasNext || !_pageController.hasClients) return;
  ...
  Widget build(BuildContext context) {
    if (_snaps.isEmpty) {
      return const Scaffold(
        ...
        body: Center(
          child: Text('No snaps', style: TextStyle(color: Colors.white)),
        ),
      );
    }
    ...
    return Scaffold(
      ...
      body: PageView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (videoIndex >= _snaps.length) return null; // Stop scrolling
          ...
          return GestureDetector(
            ...
            child: Stack(
              ...
              children: [
                ...
                // Task 6: UI Update - 3 Dots, Like, Share properly placed
                Positioned(
                  right: 16,
                  bottom: 50, // Move up a bit
                  child: Column(
                    children: [
                      _ActionButton(
